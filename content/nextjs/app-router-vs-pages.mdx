...
────────────                        ──────────
pages/                              app/
├── _app.tsx        (全局外壳)       ├── layout.tsx     (根布局)
├── _document.tsx   (HTML 骨架)     ├── page.tsx       (首页 /)
├── index.tsx       (首页 /)        ├── about/
├── about.tsx       (/about)        │   ├── layout.tsx (about 共享布局)
├── api/                            │   └── page.tsx   (/about)
│   └── posts.ts    (/api/posts)    ├── blog/
│                                   │   └── [slug]/
│                                   │       └── page.tsx (/blog/:slug)
│                                   └── api/
│                                       └── posts/
...
```text
┌─────────────────────────────────────────┐
│  服务器: 执行 Server Components          │
│          生成 RSC Payload (序列化结果)   │
│                                          │
│  客户端: 下载 HTML + RSC Payload         │
│          仅在 'use client' 节点 hydrate  │
│                                          │
│  结果: Server Components 的 JS 不发送    │
└─────────────────────────────────────────┘
```
